...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($phone)) {
        ...
        $stmt = $pdo->prepare("SELECT id, student_id, full_name FROM students WHERE student_id = ? AND phone = ?");
        ...
        if ($student) {
            ...
            $stmt = $pdo->prepare("DELETE FROM password_reset_tokens WHERE student_id = ?");
            $stmt->execute([$student['id']]);
            
            // Insert new token
            $stmt = $pdo->prepare("INSERT INTO password_reset_tokens (student_id, token, phone_number, expires_at) VALUES (?, ?, ?, ?)");
            if ($stmt->execute([$student['id'], $token, $phone, $expires])) {
                // Generate reset link
                $resetLink = "http://" . $_SERVER['HTTP_HOST'] . "/blood/student/password_reset_by_link.php?token=" . $token;
...
<head>
    ...
    <title>Forgot Password - Blood Group Management</title>
